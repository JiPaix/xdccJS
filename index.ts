  ...
  private live(ircServer: string): void {
    ...
    this.on('request', (args: { target: string; packet: string | number }) => {
      ...
      this.timeouts = setTimeout(() => {
        ...
        const err = new Error(`timeout: No response from ${args.target}`)
        if (this.path) {
        ...
        if (this.verbose) {
          console.error(
            ...
            `${colors.bold(
              colors.red('\u0058')
            )} timeout: No response from ${colors.yellow(args.target)}`
          )
        }
      }, 1000 * 15)
    })
    ...
    if (fileInfo) {
      ...
      if (fileInfo.port === 0) {
        ...
        client.on('data', data => {
          ...
          self.emit('pipe-data', data, received)
          timeout = setTimeout(() => {
            const err = new Error('connection timeout: socket hungup ')
            this.say(resp.nick, 'XDCC CANCEL')
            this.emit('pipe-err', err, fileInfo)
            if (this.verbose) {
              bar.interrupt(
